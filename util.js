import { mins, secs, start, setting } from "./script.js";
let interval;
...
export const isValidTime = () => {
    if (mins.value.toString().indexOf('.') != -1 || secs.value.toString().indexOf('.') != -1) {
        return true;
    }
    else if ((mins.value > 0 && mins.value < 60) || (secs.value > 0 && secs.value < 60)) {
        return false;
    }
    else {
        return true;
